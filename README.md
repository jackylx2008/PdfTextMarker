...
- 使用左上轴网、右下图签两张参考图辅助本地 AI 确定有效打印范围。
- 左上角由字母轴圆圈和数字轴圆圈中心线交点计算，不依赖固定页面坐标。
- 增加左侧局部二次识别和图签区域校验，阻止明显错误的边界进入拆分流程。
- 新增带彩色框、锚点坐标和分割线的 AI 边界诊断截图工具。
...
- 左上角 L/1 轴网参考截图、右下角图签参考截图，以及两端独立的 X/Y 偏移量。
- 接缝重叠、A3 打印边距和发给 AI 的预览图最长边像素。

处理流程如下：

1. 将每个源页面渲染为受限尺寸的 PNG 预览图。
2. 同时发送左上角轴网参考图、右下角图签参考图和待处理整页。
3. AI 分别返回字母轴圆圈、数字轴圆圈、图签外框及旋正角度，程序同步旋转页面和特征框。
4. 取数字轴圆圈中心的 X 与字母轴圆圈中心的 Y 形成左上锚点，以图签框右下角形成右下锚点，再分别叠加 X/Y 偏移得到最终打印范围。
...
诊断 PNG 会把 AI 结果直接画在旋正后的页面上：绿色为 L/数字轴圆圈及其交点，红色为右下图签，紫色为最终打印范围，蓝色虚线为左右 A3 分割线。若整页模型误选右侧的普通数字，程序会自动放大页面左侧进行第二次轴网识别；若交点仍落入图签区，则终止该页处理，不生成错误的拆分结果。
